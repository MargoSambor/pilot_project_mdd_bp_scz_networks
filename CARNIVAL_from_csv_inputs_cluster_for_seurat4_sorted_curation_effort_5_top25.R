...
#Run CARNIVAL
#Import datasets: 
tf_activities <- read_csv(paste0(path, tag, "dorothea_TFActivity_CARNIVALinput_top25.csv"))
...
# curation effort
omnipath_sd <- omnipath_sd %>% dplyr::filter(curation_effort > 5)
...
#save SIF
write_tsv(sif, paste0(path, tag,"omnipath_carnival_sorted_curation_efford_5.tsv"))
...
saveRDS(carnival_result, paste0(path, tag,"carnival_result_sorted_curation_efford_5.rds"))
...
#visNet
visSave(visNet, file = paste0(path, tag,'carnival_visualization_visNetwork_sorted_curation_efford_5.html'), selfcontained = TRUE)