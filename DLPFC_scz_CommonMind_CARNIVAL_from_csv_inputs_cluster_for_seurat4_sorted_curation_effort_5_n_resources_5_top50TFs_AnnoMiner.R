...
selected_brain_region="DLPFC"
selected_gender="both_genders_commonmind"
...
#Run CARNIVAL
#Import datasets: 
tf_activities <- read_csv(paste0(path, tag, "dorothea_TFActivity_CARNIVALinput_top50TFs_AnnoMiner.csv"))
...
# curation effort
omnipath_sd <- omnipath_sd %>% dplyr::filter(n_resources > 5 & curation_effort > 5)
...
#save SIF
write_tsv(sif, paste0(path, tag,"omnipath_carnival_sorted_curation_efford_5_n_resources_5_top50TFs_AnnoMiner.tsv"))


#Transcription Factor for CARNIVAL
...
saveRDS(carnival_result, paste0(path, tag,"carnival_result_sorted_curation_efford_5_n_resources_5_top50TFs_AnnoMiner.rds"))
...
#visNet
visSave(visNet, file = paste0(path, tag,'carnival_visualization_visNetwork_sorted_curation_efford_5_n_resources_5_top50TFs_AnnoMiner.html'), selfcontained = TRUE)